...
function initReviews() {
  ...
  loadMoreBtn.addEventListener('click', () => {
    ...
    // Disable button after loading all reviews
    if (currentPage >= 3) {
      loadMoreBtn.textContent = 'No more reviews to load';
  ...
  reviewSort.addEventListener('change', () => {
    loadReviews(1, reviewSort.value);
  });
...
function initTestimonialsCarousel() {
  ...
  const reviews = reviewsGrid.children;
  let currentIndex = 0;
...
function loadReviews(sortBy = 'recent') {
